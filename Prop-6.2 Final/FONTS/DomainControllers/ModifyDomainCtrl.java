...
public class ModifyDomainCtrl {
     ...
     * //@param nameSheet nombre de la hoja de la qual queremos cargar su contenido
    ...
    public void LoadSheet() throws IOException {
        ...
        for (int k = 0; k < sheets.size(); k++) {
            ...
            /*if (k > 0) CreateSheet();
            String probName = "Sheet " + k;
            if (!probName.equals(sheets.get(k))) {

                ChangeNameSheet(probName, sheets.get(k));
            }
            */
            int rows = load.size();
            if (rows < 20) {

                EraseRow(sheets.get(k), 20 - rows, 0);
            }
            else if (rows > 20) {

                AddRow(sheets.get(k), rows - 20, 0);
            }
            int cols = load.get(0).size();
            if (cols < 20) {
                EraseColumn(sheets.get(k), 20 - cols, 0);
            }
            else if (cols > 20) {
                AddColumn(sheets.get(k), cols - 20, 0);
            }
    ...
    public void deleteSheet (String nameDoc, String nameSheet) throws IOException {
        PersController.deleteSheet(nameDoc, nameSheet);
    }
